...
import KeyWrapper from "./KeyWrapper.js";

const isEvent = /^on[A-Z][A-Za-z]*/;
const hasLabel = /\:/;
...
export default class Keyboard extends Lightning.Component {
    ...
    _handleEnter() {
        const event = {
            ...
            key: this.currentKey.key.origin
        };
        if(this._inputField && this._inputField.index) {
            event.index = this._inputField.index;
        }
        if(isEvent.test(event.key)) {
            const split = event.key.split(':')
            const eventFunction = this[`${split[0]}`];
            event.key = split[1];
            if(eventFunction && eventFunction.apply && eventFunction.call) {
                eventFunction.call(this, event);
            }
            this.signal(split[0], event);
        }
        else {
    ...
    removeAt(index) {
        if(index > this._input.length - 1) {
            ...
            this._changeInput(this._input.substring(0, index) + this._input.substring(index + 1, this._input.length));
        }
    }
